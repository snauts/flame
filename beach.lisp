...
(defun simple-cross ()
  (on-top (beach-rocks 2 0 4 1) (beach-rocks 0 0 2 1)))
  ...
  (let ((cx (1+ (* w 2)))
    ...
    (box-pipe
     ...
     (dotimes (y (* 2 (1- h)) pipe)
       (let ((L-cell (cell (beach-tile (if (evenp y) 181 182))))
	     (R-cell (cell (beach-tile (if (evenp y) 189 190)))))
	 (setf pipe (place 1 (1+ y) pipe L-cell))
	 (setf pipe (place (1- cx) (1+ y) pipe R-cell))))
